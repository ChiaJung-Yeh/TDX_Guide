...
依據直線將原始公車圖資切分成數個直線段。先利用\code{st_coordinate()}將路線所有轉折點全數萃取出。演算法如圖\@ref(fig:algo-step-1)所示。
...
由以上回傳資料可知，第一個站牌鄰近 30 公尺內的候選點共 5 個，為切分點資料的第 1、2、3、4、1538 筆資料（\code{Seq}之標籤），而其距離存於\code{Dist}之標籤中。
...
地圖中紅色點表示最終匹配的點位，所有點位必定落於公車路線上。最終結果匯出如下表所示。其中\code{Dist_Cum}是站點累積里程，\code{Dist_Intv}是該站與前一站之間的路網距離。
