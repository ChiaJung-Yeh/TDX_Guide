...
# 介接高雄市停車場資料
Kaohsiung_carpark=Car_Park(access_token, county="Kaohsiung", dtype="sf")
```

```{r Car_Park_code2, echo=T, eval=F, message=F, warning=F}
# 查看Kaohsiung_carpark資料
Kaohsiung_carpark
```

```{r Car_Park_code3, echo=F, eval=T, message=F, warning=F}
datatable(Kaohsiung_carpark, options=list(pageLength=5, scrollX=T, headerCallback = DT::JS(
    "function(thead) {",
    "  $(thead).css('font-size', '0.7em');",
    "}"
  )))%>%
  formatStyle(columns=c(1:ncol(Kaohsiung_carpark)), fontSize='80%')%>%
  formatRound(c("PositionLon", "PositionLat"), digits=4)
```
