...
本套件更新於2022-03-05，套件版本為1.2.2，請確保 TDX 套件為最新版本！
:::

### 套件下載
為方便 TDX 資料之使用，透過 R 語言開發介接資料之套件，以期使用者能快速取得需要的運輸資料，並進行後續的加值應用與分析。本套件提供公共運輸基礎資料、地理編碼服務、路網地理資料服務等，並支援轉換地理資料的格式，且可直接透過函式之設定將檔案逕輸出至指定的資料夾，降低介接資料的繁瑣程序。本套件主要以 TDX 平臺中公共運輸整合資訊流通服務平臺（PTX）與交通網路地理資訊倉儲系統（GIS-T）的 API 服務為主。  
...
TDX_table=data.frame(data=c(rep("軌道", 4), rep("公車", 3), rep("自行車", 2), "航空", "觀光景點", "道路路網", "地理編碼"), type=c("軌道站點","軌道路線站點","軌道路線線型","軌道班表","公車站點","公車路線線型","公車班表","公共自行車站點","自行車路網","航空班表","觀光景點點位","道路路網線型","地理編碼服務"), source=c(rep("PTX", 11), rep("GIS-T", 2)), class=c(rep("點", 2),"線","文字","點","線","文字","點","線","文字","點","線","點"), functiona=c("[`Rail_Shape()`](#軌道路線線型資料)", "[`Rail_Station()`](#軌道站點資料)", "[`Rail_StationOfLine()`](#軌道路線站點資料)", "[`Rail_TimeTable()`](#軌道班表資料)", "[`Bus_StopOfRoute()`](#公車路線站點資料)", "[`Bus_Shape()`](#公車路線線型資料)", "[`Bus_Schedule()`](#公車班表資料)", "[`Bike_Station()`](#公共自行車站點資料)", "[`Bike_Shape()`](#自行車線型資料)","[`Air_Schedule()`](#航空班表資料)","[`ScenicSpot()`](#觀光景點點位)","[`Road_Network()`](#道路路網線型)","[`Geocoding()`](#地理編碼服務)"))
...
在使用 TDX 套件前，必須先自[公共運輸整合資訊流通服務平臺](https://ptx.transportdata.tw/PTX/Management/AccountApply)申請「一般會員」，一般會員的 API 服務呼叫次數上限為每日 20,000 次（單純資料分析已足夠使用）。進入「加入會員」的頁面後，請點選最左側的「申請一般會員」，如下圖所示，並依序輸入基本資料、服務單位、用途說明（如：學術論文/專題研究、程式教學/練習）、應用成果（申請服務概述、加值應用概述）。  

```{r TDX_member, echo=FALSE, out.width="100%", fig.align="center"}
include_graphics("./figure/TDX_member.png")
```

會員帳號啟用後，請在[會員專區](https://ptx.transportdata.tw/PTX/APIMember/ApplyGeneral)（點選網頁右上角會員專區 >> API 服務 >> API 金鑰申請）頁面申請 API 金鑰，PTX 平臺管理小組將於三個工作日核定審核結果，並寄出授權信（如下圖所示）通知申請者。授權信件中包含一 Application ID (APP ID) 及 Application Key (APP Key)，請務必保留此金鑰，後續即可在套件中使用 API 服務。  

```{r TDX_key, echo=FALSE, out.width="100%", fig.align="center"}
include_graphics("./figure/TDX_key.png")
```


### 套件版本目錄
```{r TDX_version, echo=F, eval=T}
version_table=data.frame(version=c("1.0","1.1","1.2","1.2.1","1.2.2"),
                         date=c("2022-02-08","2022-02-10","2022-02-14","2022-02-28","2022-03-05"),
                     update=c("公車、軌道、自行車之公共運輸介接函式", "加入地理編碼服務", "加入航空班表、觀光景點、道路路網介接函式", "修正地理編碼函式錯誤 Unicode亂碼問題", "修正地理編碼函式錯誤 HTTP ERROR問題"))

colnames(version_table)=c("套件版本","日期","新增/修正功能")

kable(version_table, booktabs=T)%>%
  kable_styling(bootstrap_options=c("striped", "hover"), font_size=14)%>%
  column_spec(1, bold=T)%>%
  row_spec(0, bold=T, color="white", background="#8E8E8E")
```
